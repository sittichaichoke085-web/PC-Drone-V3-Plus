    ...
    private fun showReports() {
        ...
        root.addView(
            android.widget.TextView(this).apply {
                ...
                text =
                    "ค้นหารายงานย้อนหลังตามวัน/เดือน/ปี"
        ...
        fun parseTransactions():
            ...
            if (rawSet.isEmpty()) {
                ...
                // fallback:
                // ค้นหา SharedPreferences ที่แอปเคยใช้
                try {
        ...
        fun renderReport() {
            ...
            if (report == null) {
                ...
                android.widget.Toast.makeText(
                    this@MainActivity,
                    "วันที่ไม่ถูกต้อง กรุณากรอกแบบ วัน/เดือน/ปี เช่น 07/09/2026",
            ...
            statusText.text =
                if (
                    ...
                    "วันที่ ${dateInput.text.toString().trim()} ไม่มีรายการรับ-จ่าย\n" +
                    "ยอดคงเหลือยกมา: ${money(report.opening)} บาท"

                } else {

                    "วันที่ ${dateInput.text.toString().trim()} • ${report.rows.size} รายการ\n" +
                    "รายรับ ${money(report.income)} บาท   " +
                    "รายจ่าย ${money(report.expense)} บาท   " +
                    "คงเหลือ ${money(report.closing)} บาท"
                }
        }
        ...
        fun createCsv(
            ...
            sb.append(
                "วันที่,เวลา,รายการ,รายรับ,รายจ่าย,คงเหลือ\n"
            )

            sb.append(
                "\"${dateInput.text.toString().trim()}\",,\"ยอดยกมา\",,,${money(report.opening)}\n"
            )
            ...
            report.rows.forEach { row ->
                ...
                sb.append(
                    ...
                    "\"${timeFormat.format(java.util.Date(row.time))}\"," +
                    "\"$item\"," +
                    "${if (row.income > 0.0) money(row.income) else ""}," +
                    "${if (row.expense > 0.0) money(row.expense) else ""}," +
                    "${money(row.balance)}\n"
                )
            }
        ...
        fun createPdf(
            ...
            fun header() {
                ...
                canvas.drawText(
                    "Finance report : ${dateInput.text.toString().trim()}",
                    35f,
                    y,
                    paint.apply {
                        textSize = 11f
            ...
            try {
                ...
                val uri =
                    android.net.Uri.parse(
                        "content://${packageName}.finance-reports/${file.name}"
                    )
                ...
                android.widget.Toast.makeText(
                    this@MainActivity,
                    "ไม่สามารถเปิดหน้าต่างแชร์ได้: ${e.message}",
                    android.widget.Toast.LENGTH_LONG
                ).show()
            }
        }
        ...
        // แสดงตารางของวันที่ล่าสุดที่จำไว้ทันที
        renderReport()
